...
class MBEWalkthroughController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    init(introController: UIViewController?, walkthroughType: WalkthroughType, showStatus: Bool, statusOption: WalkthroughStatusBarType, primaryColor: UIColor?, secondaryColor: UIColor?, images: [UIImage?]?, titles: [String]?) {
        ...
        if introController != nil {
            self.loggedIn = false
            self.viewController = introController!
        }
        else {
            self.loggedIn = true
        }
        ...
        if !self.pageControl.hidden {
            if self.images.count > 0 {
                if self.pageControl.currentPage+1 == self.images.count {
                    ...
                    finish.setTitle(t.uppercaseString, forState: UIControlState.Normal)
                    finish.setTitle(t.uppercaseString, forState: UIControlState.Highlighted)
                    finish.setTitle(t.uppercaseString, forState: UIControlState.Selected)
                    finish.enabled = true
                    skip.hidden = true
                }
                else {
